	...
	if(m==0) {
		if(itemSelector==0){// 1/20 확률로 부활 아이템
			if(!instance_exists(obj_item_revival))
				instance_create_depth(272,448,0,obj_item_revival)
			else
				instance_create_depth(272,448,0,obj_item_shield)
		} else if(itemSelector<=3){// 3/20 확률로 체력 상승 아이템
			instance_create_depth(272,448,0,obj_item_lifeup)
		} else if(itemSelector<=9){// 6/20 확률로 그레이즈+ 아이템
			instance_create_depth(272,448,0,obj_item_graze_plus)
		} else if(itemSelector<=13){// 4/20 확률로 축소 아이템
			instance_create_depth(272,448,0,obj_item_small)
		} else if(itemSelector<=15){// 2/20 확률로 일시정지 아이템
			instance_create_depth(272,448,0,obj_item_pause)
